...
import { deleteMemberSign } from "../../../../../api/base/general/member_sign"
import HomepageContext from "../../../../../context/HomepageContext"

const DeleteMemberSign = (props) => {
    const {id, resetData} = props
    const { setLoading } = useContext(HomepageContext)
    const [openDe, setOpenDe] = useState(false)
    const remove = async (id) => {
        setLoading(true)
        const {success, data} = await deleteMemberSign(id)
        setLoading(false)
        if (success) {
            if (data.success) {
                setOpenDe(false)
...
export default DeleteMemberSign